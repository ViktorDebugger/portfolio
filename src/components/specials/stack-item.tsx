...
const StackItem = ({
  ...
  return (
    <li className="glass-effect-button glass-main w-full overflow-hidden h-full rounded-xl border-1! border-indigo-300! shadow-2xl! shadow-indigo-900/80 backdrop-blur-xl backdrop-saturate-200 transition-all duration-300 ease-in-out hover:scale-105 md:rounded-2xl">
      <button
        onClick={() => setOpen(true)}
        className="group relative h-full w-full cursor-pointer p-1 text-black md:p-4 dark:bg-white/15! dark:text-white"
      >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            />
            <div className="fixed inset-0 flex w-screen items-end justify-center p-4 md:items-center">
              <DialogPanel
                as={motion.div}
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="glass-effect glass-main gradient-border-left-blue absolute -bottom-10 max-h-[90vh] w-full space-y-4 rounded-2xl border-2 border-l-0! border-black bg-white/40! text-black shadow-2xl! shadow-indigo-500 backdrop-blur-xl backdrop-saturate-200 md:w-5/6 dark:bg-black/40! dark:text-white"
              >
                <div className="absolute top-0 left-0 z-1 h-full w-full bg-white/30 dark:bg-black/20"></div>
                  ...
                  <div className="scrollbar-hide flex-1 overflow-x-hidden overflow-y-auto p-4 pb-16 md:p-0 md:pb-8">
                    <div className="mb-12 flex flex-col items-center justify-between gap-8 px-2 py-4 sm:px-8 md:px-8 md:py-8 lg:flex-row lg:gap-4">
                      ...
                      <div className="w-full lg:w-[65%]">
                        <DialogTitle className="mb-4 flex flex-col items-start gap-4 text-3xl font-bold sm:flex-row sm:items-end sm:gap-12 sm:text-4xl">
                          <span>{name}</span>
                          <a
                            href={documentation}
                            target="_blank"
                          ...
                          >
                            ...
                            <LinkArrow className="ml-1 inline-block size-4" />
                          </a>
                        </DialogTitle>
                    ...
                    <ul className="mx-0 my-3 flex flex-col justify-between gap-4 overflow-hidden rounded-lg border-0 border-black bg-transparent px-0 py-3 md:mx-6 md:px-6 lg:flex-row lg:gap-4 lg:bg-black/10 2xl:border dark:border-white lg:dark:bg-white/10">
